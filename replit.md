...
- **API Integration**: The platform uses a type-safe generated API client (located in `src/api/generated`) based on the backend Swagger/OpenAPI spec. The client is configured with base URL (via `VITE_API_BASE_URL` or defaults to `https://treason.nephue.com`) and Bearer token authentication.
  ...
  - **Run Backtest**: Initiated via `/v1/alpaca/runbacktest` endpoint when the Run button is clicked. Sends strategy ID, symbol, timeframe (1Min), and date range (start/end time) to execute a backtest. Returns a `runId` in the response, with success/error toast notifications and automatic cache invalidation to refresh the runs list.
