...
- **API Integration**: The platform uses a type-safe generated API client (located in `src/api/generated`) based on the backend Swagger/OpenAPI spec. The client is configured with base URL (via `VITE_API_BASE_URL` or defaults to `https://treason.nephue.com`) and Bearer token authentication.
  ...
  - **Runs**: Fetched from `/v1/alpaca/runs/search` endpoint with filters for selected strategy ID, mode type ('BACKTEST' or 'LIVE'), and startedAt date range (start of selected day to start of next day, exclusive end using `FILTER_TYPE_RANGE_EXCLUSIVE_MAX`). Response (`V1Run[]`) populates the run instance dropdown, sorted by startedAt timestamp. Each run includes metrics: profit, winRate, totalTrades (number of symbols), and sharpeRatio.
  - **Executions**: Fetched from `/v1/alpaca/executions/search` endpoint for the selected run. First queries decisions by runId, then fetches executions matching those decisionIds. Response (`V1Execution[]`) with FILLED status populates the trade history table with fillTime, fillPrice, fillQuantity, and action (derived from decision signal).
  - **Performance Metrics**: Displayed metrics (Total P/L, Win Rate, Number of Symbols, Sharpe Ratio) are sourced directly from the selected run entity's fields.
