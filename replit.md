...
- **API Integration**: The platform uses a type-safe generated API client (located in `src/api/generated`) based on the backend Swagger/OpenAPI spec. The client is configured with base URL (via `VITE_API_BASE_URL` or defaults to `https://treason.nephue.com`) and Bearer token authentication.
  ...
  - **Runs**: Fetched from `/v1/alpaca/runs/search` endpoint with filters for selected strategy ID, mode type ('BACKTEST' or 'LIVE'), and startTime date range (start of selected day to start of next day, exclusive end using `FILTER_TYPE_RANGE_EXCLUSIVE_MAX`). This filters runs by the date range being analyzed, not when the run was executed. Response (`V1Run[]`) populates the run instance dropdown, sorted by startedAt timestamp. Each run includes metrics: profit, winRate, totalTrades (number of symbols), and sharpeRatio.
  - **Executions**: Fetched from `/v1/alpaca/executions/search` endpoint filtered directly by `runId`. Queries executions and decisions in parallel for efficiency. Response (`V1Execution[]`) with FILLED status populates the trade history table with fillTime, fillPrice, fillQuantity, and action (derived from decision signal).
