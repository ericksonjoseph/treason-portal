...
- **API Integration**: The platform uses a type-safe generated API client (located in `src/api/generated`) based on the backend Swagger/OpenAPI spec. The client is configured with base URL (via `VITE_API_BASE_URL` or defaults to `https://treason.nephue.com`) and Bearer token authentication.
  ...
  - **Runs**: Fetched from `/v1/alpaca/runs/search` endpoint with filters for selected strategy ID and mode type ('BACKTEST' or 'LIVE'). No date filtering is applied - all runs for the selected strategy/type are returned. Response (`V1Run[]`) populates the run instance dropdown, sorted by startedAt timestamp. Each run includes startedAt, completedAt, and status derived from these timestamps.
